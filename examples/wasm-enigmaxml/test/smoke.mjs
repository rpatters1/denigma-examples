...
if (!modulePathArg || !musxPathArg) {
  console.error('usage: node smoke.mjs <denigma_wasm_enigmaxml.js> <input.musx>');
  process.exit(2);
}
...
try {
  const rc = Module._denigma_musx_to_enigmaxml(inputPtr, input.byteLength, outputPtrPtr, outputSizePtr, errorPtrPtr);
  if (rc !== 0) {
  ...
  if (!output.startsWith('<?xml')) {
    throw new Error('Enigma XML output is missing the XML declaration');
  }
  if (!output.includes('<finale')) {
    throw new Error('Enigma XML output is missing the <finale> root element');
  }
  if (!output.includes('</finale>')) {
    throw new Error('Enigma XML output is missing the closing </finale> tag');
  }

  console.log(`Extracted ${input.byteLength} bytes to ${outputSize} bytes of Enigma XML.`);
} finally {
