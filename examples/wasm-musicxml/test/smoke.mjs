...
if (!modulePathArg || !musxPathArg) {
  console.error('usage: node smoke.mjs <denigma_wasm_musicxml.js> <input.musx>');
  process.exit(2);
}
...
try {
  const rc = Module._denigma_musx_to_musicxml(inputPtr, input.byteLength, outputPtrPtr, outputSizePtr, errorPtrPtr);
  if (rc !== 0) {
  ...
  if (!output.includes('<score-partwise')) {
    throw new Error('MusicXML output does not contain a score-partwise element');
  }

  console.log(`Converted ${input.byteLength} bytes to ${outputSize} bytes of MusicXML.`);
} finally {
