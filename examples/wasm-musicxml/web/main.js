...
import createModule from '../../../build-wasm/examples/wasm-musicxml/denigma_wasm_musicxml.js';
...
function convert(bytes) {
  ...
  try {
    const rc = Module._denigma_musx_to_musicxml(inputPtr, bytes.byteLength, outputPtrPtr, outputSizePtr, errorPtrPtr);
    if (rc !== 0) {
...
input.addEventListener('change', async () => {
  ...
  try {
    ...
    if (!xml.includes('<score-partwise')) {
      throw new Error('Output does not look like MusicXML.');
    }
    ...
    const suffix = xml.length > PREVIEW_CHAR_LIMIT
      ...
      : '';
    status.textContent = `Converted ${file.name}: ${bytes.byteLength} MUSX bytes to ${xml.length} MusicXML characters.`;
